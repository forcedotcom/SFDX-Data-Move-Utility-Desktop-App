...
class SfdmuUtils {
     ...
     */
    static getDefaultExternalId(name, orgDescribe) {
        if (name === common_1.CONSTANTS.SFDMU.RECORD_TYPE_SOBJECT_NAME) {
            return common_1.CONSTANTS.SFDMU.DEFAULT_RECORD_TYPE_ID_EXTERNAL_ID_FIELD_NAME;
        }
        if (!orgDescribe.isInitialized) {
            return "Id";
        }
        if (common_1.CONSTANTS.SFDMU.DEFAULT_EXTERNAL_IDS[name]) {
            return common_1.CONSTANTS.SFDMU.DEFAULT_EXTERNAL_IDS[name];
        }
        const sObjectDescribe = orgDescribe.objectsMap.get(name);
        if (!sObjectDescribe || !sObjectDescribe.isDescribed) {
            return "Id";
        }
        return ([].concat([...sObjectDescribe.fieldsMap.values()].filter(field => field.nameField), [...sObjectDescribe.fieldsMap.values()].filter(field => field.autoNumber), [...sObjectDescribe.fieldsMap.values()].filter(field => field.unique))[0] || { name: "Id" })["name"];
    }
    /**
     ...
     */
    static sortFieldExpression(a, b) {
     ...
     */
    static setSOQLFields(object, fields, addIdField = false) {
     ...
     * @returns  The query string and fields. Parameters: { fields, query }.
     */
    static createQueryString(object, fields, addIdField = true, removeOffsetAndLimitAndOrderBy = false) {
         ...
         */
        const buildQueryAppendix = (object) => {
     ...
     */
    static createCountQueryString(object) {
     ...
     */
    static createLimitedQueryString(object, limit = 10) {
     ...
     */
    static validateSoql(query) {
        try {
            if (!query)
                return true;
            (0, soql_parser_js_1.parseQuery)(query);
        }
        catch (error) {
            return false;
        }
     ...
     */
    static parseSoql(soql, extractAliasesFromAggregadedFields = false) {
    ...
    */
     ...
     */
    static getAllQueryStringFields(fields, description) {
        const outputFieldsSet = new Set();
        const fieldsMap = description.fieldsMap;
        const fieldConditionsMap = new Map();
        let hasAll = false;
     ...
     */
    static excludeMultiselectFields(allFields, defaultIfEmpty = ['']) {
     ...
     */
    static _isMultiselectKeyword(keyword) {
        return keyword === 'all' || keyword.endsWith('_true') || keyword.endsWith('_false') || keyword.startsWith('type_');
    }
}
exports.SfdmuUtils = SfdmuUtils;
//# sourceMappingURL=sfdmu-utils.js.map