    ...
    setup() {
        if (global.appGlobal.wizardStep == PreviewController.wizardStep) {
            ...
            this.sfdmuCommandSetup = {
                // AUTHENTICATION
                // row 1
                sourceusername: { type: 'select', label: '--sourceusername, -s', options: sourceConnectionOptions, required: false, widthOf12: 6, helpSearchWord: 'SOURCE_USERNAME', addHelpLinks: true },
                targetusername: { type: 'select', label: '--targetusername, -t', options: targetConnectionOptions, required: true, widthOf12: 6, helpSearchWord: 'TARGET_USERNAME', addHelpLinks: true },
                // FILE AND PATH_SETTINGS
                // row 2
                path: { type: 'input', label: '--path, -p', disabled: true, required: false, widthOf12: 12, helpSearchWord: 'PATH', addHelpLinks: true },
                // LOGGING_SETTINGS
                // row 3
                quiet: { type: 'toggle', label: '--quiet', widthOf12: 3, disabled: ws.cli.silent || ws.cli.concise || ws.cli.verbose, helpSearchWord: 'QUIET', addHelpLinks: true },
                silent: { type: 'toggle', label: '--silent', widthOf12: 3, disabled: ws.cli.quiet || ws.cli.concise || ws.cli.verbose, helpSearchWord: 'SILENT', addHelpLinks: true },
                concise: { type: 'toggle', label: '--concise', widthOf12: 3, disabled: ws.cli.quiet || ws.cli.silent || ws.cli.verbose, helpSearchWord: 'CONCISE', addHelpLinks: true },
                verbose: { type: 'toggle', label: '--verbose', widthOf12: 3, disabled: ws.cli.quiet || ws.cli.silent || ws.cli.concise, helpSearchWord: 'VERBOSE', addHelpLinks: true },
                // row 4
                filelog: { type: 'select', label: '--filelog, -l', options: [{ value: '0', label: 'Off' }, { value: '1', label: 'On' }], widthOf12: 4, helpSearchWord: 'FILE_LOG', addHelpLinks: true },
                loglevel: {
                    type: 'select', label: '--loglevel', options: [{ value: 'TRACE', label: 'TRACE' },
                ...
                logfullquery: { type: 'toggle', label: '--logfullquery', widthOf12: 4, helpSearchWord: 'LOG_FULL_QUERY', addHelpLinks: true },
                // OTHER_SETTINGS
                // row 5
                json: { type: 'toggle', label: '--json', widthOf12: 3, helpSearchWord: 'JSON_FORMAT', addHelpLinks: true },
                noprompt: { type: 'toggle', label: '--noprompt, -n', widthOf12: 3, helpSearchWord: 'NO_PROMPT', addHelpLinks: true },
                nowarnings: { type: 'toggle', label: '--nowarnings, -w', widthOf12: 3, helpSearchWord: 'NO_WARNINGS', addHelpLinks: true },
                // row 6
                canmodify: { type: 'toggle', label: '--canmodify, -c', required: false, disabled: (selectedTargetConnection === null || selectedTargetConnection === void 0 ? void 0 : selectedTargetConnection.type) != common_1.ConnectionType.Org, widthOf12: 3, helpSearchWord: 'CAN_MODIFY', addHelpLinks: true },
                apiversion: { type: 'input', label: '--apiversion', required: false, widthOf12: 3, helpSearchWord: 'CLI_API_VERSION', addHelpLinks: true },
                usesf: { type: 'toggle', label: '--usesf', widthOf12: 6, helpSearchWord: 'USE_SF', addHelpLinks: true },
            };
