...
(function ($) {
    ...
    var Transfer = function (element, options) {
        ...
        this.defaults = {
            ...
            labelName: "label",
            // group data item property name
            groupItemName: "groupItem",
            // group data array property name
            groupArrayName: "groupArray",
            // data value property name
            valueName: "value",
            // item tooltip property name
            tooltipName: "tooltip",
            // items data array
            dataArray: [],
            // group data array
            ...
            totalTextTemplate: "Total: {total_num}"
        ...
        // default total number text template
        this.default_total_num_text_template = this.settings.totalTextTemplate;
        // default zero item
        ...
        this.exclamationRectangle = function (error) {
            return error && error != 'undefined' ? ("<i data-bs-html='true' data-bs-toggle='tooltip' data-bs-placement='top' title='" + error + "' data-bs-custom-class='tooltip-warning' class='fa fa-exclamation-triangle text-warning'></i>&nbsp;") : "";
        };
        // Label display with popover
        this.itemLabel = function(label, popover) {
            return label && label != 'undefined' ? `<span data-bs-html='true' data-bs-toggle='${popover == "undefined" || !popover ? null : 'tooltip'}'  data-bs-placement='top' ${popover == "undefined" || !popover ? "" : "title='" + popover + "' "} data-bs-custom-class='tooltip-info'>${label}</span>`  : "";
        }
    }
    ...
    Transfer.prototype.generate_left_items = function () {
        ...
        var labelName = this.settings.labelName;
        var valueName = this.settings.valueName;
        var tooltipName = this.settings.tooltipName;
        ...
        for (var i = 0; i < dataArray.length; i++) {
            ...
            html +=
                ...
                '<label data-tooltip="' + dataArray[i][tooltipName]  + '" data-error="' + dataArray[i][dataErrorName] + '" class="checkbox-name-' + this.id + '" for="itemCheckbox_' + i + '_' + this.id + '">' +
                this.exclamationRectangle(dataArray[i][dataErrorName]) + this.itemLabel(dataArray[i][labelName], dataArray[i][tooltipName])   +
                '</label>' +
                '</div>' +
                '</li>'
        }
        ...
        for (var i = 0; i < groupDataArray.length; i++) {
            if (groupDataArray[i][groupArrayName] && groupDataArray[i][groupArrayName].length > 0) {
                ...
                html +=
                    ...
                    '<label data-tooltip="' + groupDataArray[i][tooltipName] + '" data-error="' + groupDataArray[i][dataErrorName] + '" for="group_' + i + '_' + id + '" class="group-name-' + id + '">' +
                    this.exclamationRectangle(groupDataArray[i][dataErrorName]) +  this.itemLabel(groupDataArray[i][groupItemName], groupDataArray[i][tooltipName]) +
                    '</label>' +
                    '</div>';
                ...
                for (var j = 0; j < groupDataArray[i][groupArrayName].length; j++) {
                    ...
                    html += '<li class="transfer-double-group-list-li-ul-li transfer-double-group-list-li-ul-li-' + id + ' ' + (selected ? 'selected-hidden' : '') + '">' +
                        ...
                        '<label data-tooltip="' + groupDataArray[i][groupArrayName][j][tooltipName] + '" data-error="' + groupDataArray[i][groupArrayName][j][dataErrorName] + '" for="group_' + i + '_checkbox_' + j + '_' + id + '" class="group-checkbox-name-' + id + '">' +
                        this.exclamationRectangle(groupDataArray[i][groupArrayName][j][dataErrorName]) + this.itemLabel(groupDataArray[i][groupArrayName][j][labelName], groupDataArray[i][groupArrayName][j][tooltipName])  +
                        '</label>' +
                        '</div>' +
                        '</li>';
                }
    ...
    Transfer.prototype.generate_right_items = function () {
        ...
        for (var i = 0; i < dataArray.length; i++) {
            if (dataArray[i].selected || false) {
                html += this.generate_item(this.id, i, dataArray[i][valueName], dataArray[i][labelName], dataArray[i][tooltipName], dataArray[i][dataErrorName]);
            }
        }
        return html;
    }
                ...
                for (var j = 0; j < groupDataArray[i][groupArrayName].length; j++) {
                    if (groupDataArray[i][groupArrayName][j].selected || false) {
                        html += this.generate_group_item(this.id, i, j,
                            ...
                            groupDataArray[i][groupArrayName][j][labelName],
                            groupDataArray[i][groupArrayName][j][tooltipName],
    ...
    Transfer.prototype.move_pre_selection_group_items = function () {
        ...
        for (var i = 0; i < groupCheckboxItems.length; i++) {
            if (!groupCheckboxItems.eq(i).parent("div").parent("li").hasClass("selected-hidden") && groupCheckboxItems.eq(i).is(':checked')) {
                ...
                var tooltipText = this.$element.find(this.groupCheckboxNameLabelClass).eq(i).attr('data-tooltip');
                var value = groupCheckboxItems.eq(i).val();

                html += this.generate_group_item(this.id, groupIndex, itemIndex, value, labelText, tooltipText, dataError);
    ...
    Transfer.prototype.move_pre_selection_items = function () {
        ...
        for (var i = 0; i < checkboxItems.length; i++) {
            if (checkboxItems.eq(i).parent("div").parent("li").css("display") != "none" && checkboxItems.eq(i).is(':checked')) {
                ...
                html += self.generate_item(self.id, index, value, labelText, null, dataError);
                pre_selection_num++;
     ...
     * generate item
     */
    Transfer.prototype.generate_item = function (id, index, value, labelText, tooltipText, dataError) {
        return '<li class="transfer-double-selected-list-li  transfer-double-selected-list-li-' + id + ' .clearfix">' +
            ...
            this.exclamationRectangle(dataError) + this.itemLabel(labelText, tooltipText)  +
            '</label>' +
            '</div>' +
            '</li>';
    }
     ...
     * generate group item
     */
    Transfer.prototype.generate_group_item = function (id, groupIndex, itemIndex, value, labelText, tooltipText,  dataError) {
        return '<li class="transfer-double-selected-list-li transfer-double-selected-list-li-' + id + ' .clearfix">' +
            '<div class="checkbox-group">' +
    ...
    function get_selected_items(transfer) {
        ...
        for (var i = 0; i < transferDoubleSelectedListLiArray.length; i++) {
            ...
            var item = {};
            item[transfer.settings.labelName] = checkboxGroup.find("label").text();
