"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.UiLangSwitcherDirective = void 0;
const _1 = require(".");
function UiLangSwitcherDirective() {
    return {
        restrict: 'E',
        scope: {
            onChange: '&'
        },
        controller: _1.UiLangSwitcherController,
        controllerAs: '$ctrl',
        bindToController: true,
        template: `
		<div class="dropdown">
		  <button class="btn btn-secondary dropdown-toggle" type="button" 
		  			data-bs-toggle="dropdown" 
					aria-haspopup="true" 
					aria-expanded="false"
					ng-click="$ctrl.expandDropdown()">
			<span>
				<i class="{{$ctrl.selectedLang.flagClass}}"></i>
				{{$ctrl.selectedLang.nativeName}}
			</span>
		  </button>
		  <div class="dropdown-menu">
			<a class="dropdown-item" ng-repeat="lang in $ctrl.selectSource" 
				ng-click="$ctrl.changeLanguage(lang.code)" 
				href="javascript:void(0)">
			  <span class="{{lang.flagClass}}"></span> {{lang.nativeName}}
			</a>
		  </div>
		</div>
	  `,
    };
}
exports.UiLangSwitcherDirective = UiLangSwitcherDirective;
//# sourceMappingURL=uiLangSwitcher.directive.js.map