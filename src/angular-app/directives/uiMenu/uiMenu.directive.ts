...
import { IActionEventArgParam, IMenuItem } from "../../../models";
...
export const uiMenuDirective = ($compile: angular.ICompileService, $app: IAppService) => {
	return {
		restrict: 'E',
		scope: {
			navbarClass: '@',
		...
		link: ($scope: IUiMenuScope, $element: angular.IAugmentedJQuery, $attrs: angular.IAttributes) => {
			...
			$scope.itemClickHandler = (menuId: string, hasChildren: boolean) => {
				...
				if (item) {
					...
					} as IActionEventArgParam<IMenuItem>);
				}
			};

			const renderItems = (source: IMenuItem[], level: number) => {

				let html = '';

				source.forEach(menuItem => {

					menuItem.id ||= CommonUtils.randomString();
			...
			$app.$broadcast.onAction('setSource', 'uiMenu', (args: IActionEventArgParam<IMenuItem>) => {
